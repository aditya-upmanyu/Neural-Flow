...
import { validateMode, validateAttackParams } from './middleware/validation.js';
...
let systemMode    = 'AI';
let mainLoopIntervalId = null;  // Store for cleanup
let broadcastStateIntervalId = null;  // Store for cleanup
...
function runAIDecisionEngine() {
  ...
  if (incident.state === 'NORMAL') {
    ...
    if (degradedNode) {
      ...
      logger.info(`[DIAG AI] Incident DETECTED on node ${degradedNode.nodeId} (${degradedNode.name}): latency=${st.latency}ms, rps=${st.requestsPerSecond}, slope=${slope.toFixed(1)} [env=${activeEnvironment}, mode=${systemMode}]`);
    ...
    if ((hasPrediction || alreadyBreached) && hasEnoughHistory) {
      ...
      logger.info(`[DIAG AI] Incident PREDICTED on node ${node.nodeId} (${node.name}): breachNote=${breachNote}, predictedBreach=${incident.predictedBreach}`);
    ...
    if (systemMode === 'AI') {
      ...
      logger.info(`[DIAG AI] Incident PREDICTED -> REROUTING. Candidate nodes: [${candInfo}]`);
      ...
      setTimeout(() => {
        if (rerouteGeneration !== capturedGeneration) {
          logger.info(`[DIAG AI] Reroute generation mismatch: captured=${capturedGeneration}, current=${rerouteGeneration}`);
          return;
        }
        logger.info(`[DIAG AI] Calling executeAIReroute for node ${node.nodeId} (${node.name})`);
    ...
    // FIX 1: Stop the load generator and clear isUnderAttack on all nodes.
    // Without this, the attacked node stays permanently red ("under attack")
    // and the continued high-RPS load cascades to Node 2/3 after reroute.
    const attackedNodeId = incident.nodeId;
    loadGenerator.stop();
    activeNodes.forEach(n => n.endAttack());
    broadcastToAll({ type: 'attack_end', data: { nodeId: attackedNodeId, source: 'auto_recovery' } });

    // FIX 2: Restore traffic weights to equal distribution after recovery.
    // Without this, the rerouted node (e.g. Node 2) stays at 65% weight permanently,
    // causing it to degrade and trigger a false cascade incident on Node 2/3.
    const equalWeight = Math.floor(100 / activeNodes.length);
    const remainder   = 100 - equalWeight * activeNodes.length;
    activeNodes.forEach((n, i) => {
      n.metrics.traffic = equalWeight + (i === 0 ? remainder : 0);
    });
    eventStore.addEvent('INFO', null,
      `⏹ Load generator stopped. All nodes cleared of attack state after recovery.`, 'LOW');
    broadcastToAll({ type: 'event', data: eventStore.getAllEvents()[0] });
    eventStore.addEvent('INFO', null,
      `↺ Traffic weights restored to equal distribution (${activeNodes.map(n => n.metrics.traffic + '%').join(' / ')}) after recovery.`, 'LOW');
    broadcastToAll({ type: 'event', data: eventStore.getAllEvents()[0] });
    ...
    incident.cooldownExpiry = Date.now() + 5000;
    incident.nodeId         = null; // Clear attacked nodeId so UI does not treat node as attacked during cooldown
    broadcastState();
  }
...
function executeAIReroute(fromNode, predictedBreach) {
  ...
  logger.info(`[DIAG AI] executeAIReroute CALLED: fromNode=${fromNode.nodeId} (${fromNode.name}), lastRerouteTime=${lastRerouteTime}, elapsed=${now - lastRerouteTime}ms, cooldown=${REROUTE_COOLDOWN_MS}ms, loadGeneratorActive=${loadGenerator.isActive}`);
  ...
  logger.info(`[DIAG AI] Candidates evaluated: count=${candidates.length}, list=[${candidates.map(c => `${c.nodeId}:${c.name}(health=${c.metrics.health})`).join(', ')}]`);
  ...
  logger.info(`[DIAG AI] Traffic weights shifted. Before: ${JSON.stringify(beforeWeights)} | After: ${JSON.stringify(afterWeights)}`);
...
mainLoopIntervalId = setInterval(mainLoop, 100);
broadcastStateIntervalId = setInterval(broadcastState, 1000);
...
// Mode switch
app.post('/api/mode', validateMode, (req, res) => {
...
// Stop attack / contain traffic
app.post('/api/attack/stop', (req, res) => {
  console.log('⏹ Stop load/attack requested');
  loadGenerator.stop();
  activeNodes.forEach(n => n.endAttack());

  // Reset traffic weights to equal distribution
  const equalWeight = Math.floor(100 / activeNodes.length);
  const remainder   = 100 - equalWeight * activeNodes.length;
  activeNodes.forEach((n, i) => {
    n.metrics.traffic = equalWeight + (i === 0 ? remainder : 0);
  ...
  manualModeState.attackedNodeId = null;
  activeIncidentDetails = null;

  eventStore.addEvent('INFO', null, '⏹ Attack stopped / traffic contained by operator.', 'LOW');
  broadcastToAll({ type: 'event', data: eventStore.getAllEvents()[0] });
  broadcastToAll({ type: 'attack_end', data: { nodeId: null } });
...
async function gracefulShutdown(signal) {
  ...
  try {
    ...
    // Clear main loop intervals to prevent zombie process
    if (mainLoopIntervalId) {
      clearInterval(mainLoopIntervalId);
      mainLoopIntervalId = null;
      console.log('?  Stopped main loop interval');
    }
    if (broadcastStateIntervalId) {
      clearInterval(broadcastStateIntervalId);
      broadcastStateIntervalId = null;
      console.log('?  Stopped broadcast interval');
    }
